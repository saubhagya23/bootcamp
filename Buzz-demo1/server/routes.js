...
app.post('/buzz',upload.single('buzzImage'),(req,res,next) => {console.log('buzz data----',req.file, req.body); next();},buzzController.createBuzz);

app.get('/buzz',(req,res,next) => { console.log('buzz fetch ------------'); next();}, buzzController.fetchAllBuzz);

app.get('/user',(req,res,next) => { console.log(req.user,'----middleware ---in get user');console.log("request.session----",req.session);next();},userController.getUserSession);
